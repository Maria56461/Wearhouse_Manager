...
//mi-a mers incarcarea robotului
...
/*snprintf(buf, LOG_BUFFER_LEN-1,"%c %c \n",
				parkinglot->arrived_trucks->next->destination[0],
				parkinglot->arrived_trucks->next->next->destination[0]);
		logger(output_path, TRUCK_ARRIVED, buf); */
	...
	/*			snprintf(buf, LOG_BUFFER_LEN-1,"%c %c %c\n",
				parkinglot->arrived_trucks->next->destination[0],
				parkinglot->arrived_trucks->next->next->destination[0],
				parkinglot->arrived_trucks->next->next->next->destination[0]);
